...
export default function SIIConnectionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    setProgress(0);
    setProgressMessage('Conectando con SII...');
    ...
    try {
      const response = await fetch('/api/sii/auth', {
        method: 'POST',
      ...
      if (!data.success) {
        setError(data.error || 'Error al conectar con SII');
        setLoading(false);
        setProgress(0);
        setProgressMessage('');
        return;
      ...
      // Success!
      setProgress(100);
      setProgressMessage('¡Conexión exitosa!');

      // Small delay before redirect
      setTimeout(() => {
        if (data.needs_setup) {
          // Pass company_id to setup page so it knows which company to configure
          router.push(`/onboarding/setup?companyId=${data.company_id}`);
        } else {
      ...
      clearInterval(progressInterval);
      setError('Error al conectar. Por favor intenta nuevamente.');
      setLoading(false);
      setProgress(0);
      setProgressMessage('');
            ...
            <div className="mb-6 flex justify-center">
              ...
              >
                ...
                <div className="flex flex-col items-center justify-center mx-2 h-8">
                  {loading ? (
                    <svg className="h-6 w-6 text-emerald-500 animate-spin" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                    </svg>
                  ) : (
                    <>
                      <svg className="h-4 w-8 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3" />
                      </svg>
                      <svg className="h-4 w-8 text-emerald-500 -mt-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
