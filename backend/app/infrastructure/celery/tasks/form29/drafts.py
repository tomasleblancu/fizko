...
def generate_f29_drafts_all_companies(
    ...
    for all companies with active subscriptions. Also fetches SII proposal for each draft.
    Designed for monthly batch processing.
    ...
    try:
        ...
        logger.info(
            ...
            f"fetch_sii_proposal={fetch_sii_proposal}"
        )
...
def generate_f29_draft_for_company(
    ...
    for one company. Also fetches SII proposal automatically.
    Used when user requests F29 calculation on-demand.
    ...
    Args:
        ...
        auto_calculate: Whether to auto-calculate values from tax documents (default: True)
        fetch_sii_proposal: Whether to fetch SII proposal (default: True)
