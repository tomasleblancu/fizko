...
export function SIIConnectionPage() {
  const navigate = useNavigate();
  const { signOut } = useAuth();
  const { saveSIICredentials, needsOnboarding } = useSession();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await saveSIICredentials({ rut: rut.trim(), password });
      // Success - useSession will update needsOnboarding and we'll redirect
