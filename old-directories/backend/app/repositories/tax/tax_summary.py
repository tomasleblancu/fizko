...
class TaxSummaryRepository:
    ...
    async def get_tax_summary(
        ...
        # Calculate PPM as 0.125% of net revenue (excluding IVA)
        net_revenue = total_revenue - iva_collected
    ...
    def _calculate_ppm(self, net_revenue: float) -> Optional[float]:
        """
        Calculate PPM (Pago Provisional Mensual) as 0.125% of net revenue (excluding IVA).

        PPM is a monthly provisional payment towards annual income tax.
        It's calculated as 0.125% of the net sales revenue (total - IVA) for the period.

        Args:
            net_revenue: Net sales revenue for the period (excluding IVA)

        Returns:
            PPM amount (0.125% of net revenue) or None if revenue is 0
        """
        # Calculate PPM as 0.125% of net revenue
        if net_revenue > 0:
